...
class Picking(models.Model):
	...
	origin_id = fields.Many2one("stock.picking", "Pickin de origen", help="Origen de devolución")
	devolucion_ids = fields.One2many("stock.picking", "origin_id", "Devoluciones")
	...
	def tp_recalcular_origen(self):
		registros = self.env['stock.picking'].search([])
		for item in registros:
			if item.origin and len(item.origin.split(" ")) > 1:
				partes = item.origin.split(" ")
				nombre = partes[len(partes) - 1]
				origen = self.env['stock.picking'].search([('name', '=', nombre)])
				if origen:
					item.write({'origin_id': origen.id})

	def name_get(self):
		""" Display 'Warehouse_name: PickingType_name' """
		res = []
		for picking_type in self:
			if self._context.get('es_cpe'):
				name = picking_type.pe_number or picking_type.name
			else:
	...
	def _get_pdf417_code(self):
		for picking_id in self:
			...
			if picking_id.pe_guide_number and picking_id.pe_is_eguide:
				...
				image.save(tmpf, 'png')
				# tmpf.seek(0)
				picking_id.sunat_pdf417_code = encodebytes(tmpf.getvalue())
			else:
	...
	def _compute_get_qr_code(self):
		for guia in self:
			...
			if not all((guia.name != '/', guia.pe_is_eguide, qr_mod)):
				...
				tmpf = BytesIO()
				image.save(tmpf, 'png')
				guia.sunat_qr_code = encodebytes(tmpf.getvalue())
			else:
				guia.sunat_qr_code = ''
