...
class AccountMove(models.Model):
	...
	devoluciones_ocultar = fields.Many2many("stock.picking", "factura_devolucion_guia", "factura_id", "devolucion_id", string="Ocultar devueltos", domain="[('id', 'in', pe_stock_ids), ('devolucion_ids', '!=', False)]")#

	@api.model
	def _get_pe_type_operation(self):
		return self.env['pe.datas'].get_selection("PE.TABLA12")

	@api.depends("pe_stock_ids", "devoluciones_ocultar")
	def _get_eguide_name(self):
		for invoice in self:
			name = []
			for pe_stock_id in invoice.pe_stock_ids:
				if pe_stock_id.id not in invoice.devoluciones_ocultar.ids and pe_stock_id.pe_guide_number and pe_stock_id.pe_guide_number != "/":
