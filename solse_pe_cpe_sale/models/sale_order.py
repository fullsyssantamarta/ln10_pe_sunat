...
# import odoo.addons.decimal_precision as dp  # No usado en Odoo 18
...
class SaleOrder(models.Model):
	...
	amount_discount = fields.Monetary(string='Descuento', store=True, readonly=True, compute='_amount_all', tracking=True)
	hide_send_buttons = fields.Boolean(compute='_compute_hide_send_buttons')

	@api.depends_context('uid')
	def _compute_hide_send_buttons(self):
		has_group = self.env.user.has_group('solse_pe_cpe_sale.group_hide_sale_send_buttons')
		for record in self:
			record.hide_send_buttons = has_group
