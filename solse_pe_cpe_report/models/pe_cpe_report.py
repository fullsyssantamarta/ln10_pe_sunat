...
class SolsePeCpeReport(models.Model):
	...
	def _generate_xlsx_base64_bytes(self, txt_string, sheet_name, headers=[], custom_format_dict=dict()) :
		...
		version_panda = pandas.__version__.split(".")[0]
		if version_panda == 1:
			xlsx_writer.save()
		else:
			xlsx_writer.close()
	...
	def enviar_mensaje(self):
		if not self.factura_ids:
			raise UserError('No hay facturas en el reporte. Primero debe generar el reporte.')

		if not self.partner_id:
			raise UserError('No hay contactos seleccionados para el envío.')
		...
		if len(destinos) == 0:
			raise UserError('Los contactos seleccionados no tienen correo electrónico configurado.')

		account_mail = self.obtener_datos_correo()
		context = account_mail.get('context')
		if not context:
			raise UserError('No se pudo obtener la configuración del correo.')
		template_id = account_mail['context'].get('default_template_id')
		if not template_id:
			raise UserError('No se encontró la plantilla de correo electrónico.')
		attachment_ids = []
