...
class CPE:
	...
	def _agregar_informacion_general_comprobante(self, invoice_id):
		...
		# 17 Cantidad de items
		lines_cpe = invoice_id.invoice_line_ids.filtered(lambda ln: ln.price_subtotal >= 0 and ln.display_type not in ('line_section', 'line_note') and getattr(ln, 'is_rounding_line', False) == False)
		tag = etree.QName(self._cbc, 'LineCountNumeric')
		etree.SubElement((self._root), (tag.text), nsmap={'cbc': tag.namespace}).text = str(len(lines_cpe))
	...
	def _agregar_informacion_empresa_2_0(self, invoice_id):
		...
		district = invoice_id.company_id.partner_id.l10n_pe_district
		etree.SubElement(address, (tag.text), nsmap={'cbc': tag.namespace}).text = district.code if district else '0000'
	...
	def _agregar_informacion_empresa(self, invoice_id):
		...
		tag = etree.QName(self._cbc, 'ID')
		ubigeo = datos_entidad['ubigeo'] or '000000'
		etree.SubElement(address, (tag.text), schemeAgencyName='PE:INEI', schemeName='Ubigeos', nsmap={'cbc': tag.namespace}).text = str(ubigeo)
		tag = etree.QName(self._cbc, 'AddressTypeCode')
	...
	def _agregar_informacion_de_impuestos(self, invoice_id):
		...
		datos = invoice_id.tax_totals

		# DEBUG: Log para diagnóstico
		_logging.info('='*80)
		_logging.info('DIAGNÓSTICO DE IMPUESTOS - Factura: %s' % invoice_id.name)
		_logging.info('tax_totals completo: %s' % datos)
		_logging.info('='*80)
		...
		amount_by_group = []
		if datos and "groups_by_subtotal" in datos:
			datos_sub = datos["groups_by_subtotal"]
			_logging.info('Estructura groups_by_subtotal encontrada')
			_logging.info('Claves disponibles en groups_by_subtotal: %s' % list(datos_sub.keys()))
			# Intentar con diferentes nombres de claves según el idioma
			importe_libre = (
				datos_sub.get("Importe libre de impuestos", False) or
				datos_sub.get("Untaxed Amount", False) or
				datos_sub.get("Importe sin impuestos", False) or
				datos_sub.get("Base imponible", False)
			)
			if not importe_libre:
				_logging.info('No se encontró clave estándar, buscando primera clave disponible...')
				for reg in datos_sub:
					importe_libre = datos_sub[reg]
					_logging.info('Usando clave alternativa: %s' % reg)
					break
			amount_by_group = importe_libre or []
			_logging.info('amount_by_group obtenido: %s' % amount_by_group)
		elif datos:
			_logging.info('Estructura subtotals encontrada (Odoo 15+)')
			for subtotal in datos.get('subtotals', []):
				for group in subtotal.get('tax_groups', []):
					group_copy = group.copy()
					if 'tax_group_amount' not in group_copy:
						group_copy['tax_group_amount'] = group_copy.get('tax_amount_currency', group_copy.get('tax_amount', 0.0))
					if 'tax_group_base_amount' not in group_copy:
						# En Odoo 18, la clave es 'base_amount_currency' o 'base_amount', no 'tax_base_amount'
						group_copy['tax_group_base_amount'] = group_copy.get('base_amount_currency', group_copy.get('base_amount', 0.0))
					if 'tax_group_id' not in group_copy:
						group_copy['tax_group_id'] = group_copy.get('id')
					amount_by_group.append(group_copy)
			_logging.info('amount_by_group obtenido: %s' % amount_by_group)

		# FALLBACK: Si amount_by_group sigue vacío, calcularlo manualmente
		if not amount_by_group:
			_logging.warning('tax_totals no proporcionó datos, calculando manualmente...')
			# Obtener impuestos de las líneas de factura
			tax_groups_dict = {}
			for line in invoice_id.invoice_line_ids:
				for tax in line.tax_ids:
					if tax.tax_group_id.id not in tax_groups_dict:
						tax_groups_dict[tax.tax_group_id.id] = {
							'tax_group_id': tax.tax_group_id.id,
							'tax_group_name': tax.tax_group_id.name,
							'tax_group_amount': 0.0,
							'tax_group_base_amount': 0.0,
						}
					# Calcular impuesto para esta línea
					price_unit = line.price_unit * (1 - (line.discount or 0.0) / 100.0)
					tax_values = tax.compute_all(price_unit, currency=invoice_id.currency_id,
						quantity=line.quantity, product=line.product_id, partner=invoice_id.partner_id)
					tax_groups_dict[tax.tax_group_id.id]['tax_group_base_amount'] += tax_values['total_excluded']
					if tax_values.get('taxes'):
						tax_groups_dict[tax.tax_group_id.id]['tax_group_amount'] += tax_values['taxes'][0]['amount']
			amount_by_group = list(tax_groups_dict.values())
			_logging.info('amount_by_group calculado manualmente: %s' % amount_by_group)
		...
		# 23
		_logging.info('Iniciando generación de TaxSubtotal. Total de grupos: %s' % len(amount_by_group))
		for group_tax in amount_by_group:
			group_id = group_tax['tax_group_id']
			base_tax_line = group_tax['tax_group_base_amount']
			amount_tax_line = group_tax['tax_group_amount']
			_logging.info('Procesando grupo: ID=%s, Base=%s, Monto=%s' % (group_id, base_tax_line, amount_tax_line))
			tax = invoice_id.line_ids.mapped('tax_ids').filtered(lambda x: x.tax_group_id.id == group_id)
			tax = tax and tax[0]
			if not tax:
				_logging.warning('No se encontró impuesto para el grupo %s' % group_id)
				continue
			_logging.info('Impuesto encontrado: %s (código: %s, tipo: %s)' % (tax.name, tax.l10n_pe_edi_tax_code, tax.pe_tax_type.code if tax.pe_tax_type else 'None'))
			if tax.pe_tax_type and tax.pe_tax_type.code == constantes.IMPUESTO['gratuito']:
				_logging.info('Impuesto gratuito, saltando...')
				continue
			elif tax.pe_tax_type and tax.pe_tax_type.code == constantes.IMPUESTO['inafecto']:
				_logging.info('Impuesto inafecto, saltando...')
				continue
			else:
				if base_tax_line == 0:
					_logging.info('Base de impuesto es 0, saltando...')
					continue

				base_tax_line = base_tax_line - abs(total_base_negativo)
				_logging.info('Generando TaxSubtotal para %s (base ajustada: %s)' % (tax.name, base_tax_line))
				...
				etree.SubElement(category, (tag.text), nsmap={'cbc': tag.namespace}).text = str(round_up(tax.amount, 2))
	...
	def _agregar_informacion_montos_totales(self, invoice_id):
		...
		if tiene_anticipos:
			etree.SubElement(total, (tag.text), currencyID=(invoice_id.currency_id.name), nsmap={'cbc': tag.namespace}).text = "%.2f" % prepaid_amount_untaxed
		else:
			etree.SubElement(total, (tag.text), currencyID=(invoice_id.currency_id.name), nsmap={'cbc': tag.namespace}).text = "%.2f" % amount_untaxed
		...
		if tiene_anticipos:
			etree.SubElement(total, (tag.text), currencyID=(invoice_id.currency_id.name), nsmap={'cbc': tag.namespace}).text = "%.2f" % prepaid_amount
		else:
			etree.SubElement(total, (tag.text), currencyID=(invoice_id.currency_id.name), nsmap={'cbc': tag.namespace}).text = "%.2f" % amount_total
		...
		total_redondeo = 0
		for line in invoice_id.invoice_line_ids.filtered(lambda ln: getattr(ln, 'is_rounding_line', False) == True):
	...
	def _agregar_informacion_lineas_comprobante(self, invoice_id):
		...
		digits = decimal_precision_obj.precision_get('Product Price') or 2
		bases_json = {}
		for line in invoice_id.invoice_line_ids.filtered(lambda ln: ln.price_subtotal >= 0 and ln.display_type not in ('line_section', 'line_note') and getattr(ln, 'is_rounding_line', False) == False):
			...
			if price_unit_all > 0 and line.pe_affectation_code not in ['30', '31', '32', '33', '34', '35', '36']:
				...
				if price_unit_all == 0 or invoice_id.pe_invoice_code in ('07', '08'):
					etree.SubElement(alternative, (tag.text), currencyID=(invoice_id.currency_id.name), nsmap={'cbc': tag.namespace}).text = str(round_up(float_round(price_unit_all, digits), digits)) #str(round(float_round(price_unit_all, digits), 10))
	...
	def _agregar_informacion_tipo_transaccion(self, invoice_id):
		if invoice_id.invoice_payment_term_id and invoice_id.invoice_payment_term_id.tipo_transaccion == 'credito' or invoice_id.invoice_date_due > invoice_id.invoice_date:
			...
			etree.SubElement(payment_terms, (tag.text), currencyID=(invoice_id.currency_id.name), nsmap={'cbc': tag.namespace}).text = "%.2f" % amount_total
			...
			for cuota in cuotas_pago:
				...
				tag = etree.QName(self._cbc, 'Amount')
				etree.SubElement(payment_terms_cuota, (tag.text), currencyID=(invoice_id.currency_id.name), nsmap={'cbc': tag.namespace}).text = "%.2f" % cuota['amount']
		...
		for journal_id in journal_ids:
			...
			for invoice_id in batch.summary_ids.filtered(lambda inv: inv.journal_id.id == journal_id.id).sorted(key=(lambda r: r.name)):
				if invoice_id.pe_cpe_id.state in ('draft', 'cancel'):
					...
					taxes = []
					taxes.append({'amount_tax_line':0,  'tax_name': constantes.IMPUESTO['igv'],  'tax_description':'IGV',  'tax_type_pe':'VAT', 'percent': 0})
					taxes.append({'amount_tax_line':0,  'tax_name':constantes.IMPUESTO['isc'],  'tax_description':'ISC',  'tax_type_pe':'EXC', 'percent': 0})
					taxes.append({'amount_tax_line':0,  'tax_name':'9999',  'tax_description':'OTR',  'tax_type_pe':'OTH', 'percent': 0})
					taxes.append({'amount_tax_line':0,  'tax_name': constantes.IMPUESTO['ICBPER'],  'tax_description':'ICBPER',  'tax_type_pe':'OTH', 'percent': 0})
					...
					if datos and "groups_by_subtotal" in datos:
						datos_sub = datos["groups_by_subtotal"]
						importe_libre = datos_sub.get("Importe libre de impuestos", False)
						if not importe_libre:
							for reg in datos_sub:
								importe_libre = datos_sub[reg]
								break
						...
						for subtotal in datos.get('subtotals', []):
							for group in subtotal.get('tax_groups', []):
								group_copy = group.copy()
								if 'tax_group_amount' not in group_copy:
									group_copy['tax_group_amount'] = group_copy.get('tax_amount', 0.0)
								if 'tax_group_base_amount' not in group_copy:
									group_copy['tax_group_base_amount'] = group_copy.get('tax_base_amount', 0.0)
								if 'tax_group_id' not in group_copy:
									group_copy['tax_group_id'] = group_copy.get('id')
								amount_by_group.append(group_copy)
					...
					etree.SubElement(status, (tag.text), nsmap={'cbc': tag.namespace}).text = invoice_id.pe_condition_code or '1'
				...
				for tax in taxes:
					if tax['tax_name'] == '9999' and tax['amount_tax_line'] > 0:
						...
						category = etree.SubElement(tax_subtotal, (tag.text), nsmap={'cac': tag.namespace})
						tag = etree.QName(self._cbc, 'Percent')
						etree.SubElement(category, (tag.text), nsmap={'cbc': tag.namespace}).text = str(round_up(tax['percent'], 2))
						tag = etree.QName(self._cac, 'TaxScheme')
						scheme = etree.SubElement(category, (tag.text), nsmap={'cac': tag.namespace})
						tag = etree.QName(self._cbc, 'ID')
