...
class AccountMove(models.Model):
	...
	pe_error_code = fields.Char(string='Error Code', related='pe_cpe_id.error_code', readonly=True)
	...
	origin_doc_code = fields.Selection('_get_origin_doc_code', 'Código de documento de origen',
	  compute='_compute_origin_doc')
	origin_doc_number = fields.Char('Número de documento de origen', compute='_compute_origin_doc')
	...
	pe_additional_number = fields.Char(string='Número adicional')
	pe_export_amount = fields.Monetary('Monto de exportación', compute='_pe_compute_operations')
	pe_sunat_transaction = fields.Selection('_get_pe_pe_sunat_transaction', string='Transacción SUNAT', default='01')
	...
	pe_license_plate = fields.Char('Placa', size=10, copy=False)
	...
	total_operaciones_gravadas = fields.Monetary(store=False, readonly=True,compute='_compute_amount_reporte', tracking=True)
	total_operaciones_gravadas_dolar = fields.Monetary(store=False, readonly=True,compute='_compute_amount_reporte', tracking=True)
	total_operaciones_exoneradas = fields.Monetary(store=False, readonly=True,compute='_compute_amount_reporte', tracking=True)
	total_operaciones_exoneradas_dolar = fields.Monetary(store=False, readonly=True,compute='_compute_amount_reporte', tracking=True)
	total_operaciones_gratuitas = fields.Monetary(store=False, readonly=True, compute='_compute_amount_reporte', tracking=True)
	total_operaciones_gratuitas_dolar = fields.Monetary(store=False, readonly=True, compute='_compute_amount_reporte', tracking=True)
	total_operaciones_inafectas = fields.Monetary(store=False, readonly=True,compute='_compute_amount_reporte', tracking=True)
	total_operaciones_inafectas_dolar = fields.Monetary(store=False, readonly=True,compute='_compute_amount_reporte', tracking=True)
	total_operaciones_exportadas = fields.Monetary(store=False, readonly=True, compute='_compute_amount_reporte', tracking=True)
	total_operaciones_exportadas_dolar = fields.Monetary(store=False, readonly=True, compute='_compute_amount_reporte', tracking=True)
	...
	def get_views(self, views, options=None):
		...
		view_types = {v[1] for v in views}
		if 'form' not in view_types:
			return res
		...
		if move_type == 'in_refund':
			...
			if arch:
				...
				for el in root.iter('field'):
					if el.attrib.get('name') == 'payment_reference':
						el.attrib.update({'string': 'Nota de crédito del proveedor'})
						break
				form_view['arch'] = ET.tostring(root, encoding='unicode', method='xml')
		...
		if paso_validacion:
			try:
				line_tree = form_view.get('fields', {}).get('invoice_line_ids', {}).get('views', {}).get('list', {})
				if not line_tree:
					line_tree = form_view.get('fields', {}).get('invoice_line_ids', {}).get('views', {}).get('tree', {})
				if line_tree and line_tree.get('arch'):
					xml_lines = etree.XML(line_tree['arch'].encode('utf-8') if isinstance(line_tree['arch'], str) else line_tree['arch'])
					nodes = xml_lines.xpath("//field[@name='pe_affectation_code']")
					if nodes:
						nodes[0].set('column_invisible', '0')
						line_tree['arch'] = etree.tostring(xml_lines, encoding='unicode', method='xml')
			except Exception:
				pass
	...
	def _compute_amount_reporte(self,detener=False):
		...
		"""
		for reg in self:
			# Solo procesar si es una factura/nota de crédito (no pagos ni otros tipos)
			if reg.move_type not in ('out_invoice', 'out_refund', 'in_invoice', 'in_refund'):
				reg.total_operaciones_gravadas = 0.0
				reg.total_operaciones_gravadas_dolar = 0.0
				reg.total_operaciones_exoneradas = 0.0
				reg.total_operaciones_exoneradas_dolar = 0.0
				reg.total_operaciones_gratuitas = 0.0
				reg.total_operaciones_gratuitas_dolar = 0.0
				reg.total_operaciones_inafectas = 0.0
				reg.total_operaciones_inafectas_dolar = 0.0
				reg.total_operaciones_exportadas = 0.0
				reg.total_operaciones_exportadas_dolar = 0.0
				continue
			try:
	...
	def _compute_discount(self):
		for reg in self:
			...
			try:
				# Solo procesar si es una factura/nota de crédito (no pagos ni otros tipos)
				if reg.move_type not in ('out_invoice', 'out_refund', 'in_invoice', 'in_refund'):
					reg.pe_total_discount = 0.0
					reg.pe_total_discount_tax = 0.0
					reg.pe_amount_discount = 0.0
					continue
				...
				for line in reg.invoice_line_ids:
					# En Odoo 18, is_rounding_line ya no existe, usar getattr para compatibilidad
					if getattr(line, 'is_rounding_line', False) == True:
						continue
					# Proteger acceso a price_total durante creación de registros
					try:
	...
	def _get_pdf417_code(self):
		for invoice_id in self:
			res = []
			if invoice_id.name and invoice_id.l10n_latam_document_type_id.is_cpe and invoice_id.l10n_latam_document_number:
				...
				tmpf = BytesIO()
				image.save(tmpf, 'png')
				invoice_id.sunat_pdf417_code = encodebytes(tmpf.getvalue())
	...
	def validate_sunat_invoice(self):
		...
		for line in self.invoice_line_ids.filtered(lambda ln: getattr(ln, 'is_rounding_line', False) == False):
			if line.display_type in ('line_section', 'line_note'):
				continue
			if line.display_type not in ('line_section', 'line_note') and (line.quantity == 0.0 or line.price_unit == 0.0):
		...
		# Si el número tiene un prefijo (ejm: '01 F001-1'), nos quedamos con la parte del correlativo
		if ' ' in document_number:
			document_number = document_number.split(' ')[-1]

		if not re.match('^(B|F){1}[A-Z0-9]{3}\\-\\d+$', document_number):
			raise UserError('El numero de la factura ingresada (%s) no cumple con el estandar.\nVerificar la secuencia del Diario por jemplo F001- o BN01-. \nPara cambiar ir a Configuracion/Contabilidad/Diarios/Secuencia del asiento. \nNombre: %s' % (self.l10n_latam_document_number, self.name))
	...
	def obtener_validacion_fecha(self):
		return True

	def _post(self, soft=True):
		validacion = self.obtener_validacion_fecha()
		res = super(AccountMove, self)._post(soft=validacion)
		for invoice_id in self:
	...
	def _pe_compute_operations(self):
		for invoice_id in self:
			...
			# Solo procesar si es una factura/nota de crédito (no pagos ni otros tipos)
			if invoice_id.move_type not in ('out_invoice', 'out_refund', 'in_invoice', 'in_refund'):
				invoice_id.pe_taxable_amount = 0.0
				invoice_id.pe_exonerated_amount = 0.0
				invoice_id.pe_unaffected_amount = 0.0
				invoice_id.pe_free_amount = 0.0
				invoice_id.pe_export_amount = 0.0
				invoice_id.pe_amount_tax = 0.0
				continue
