...
class PeruSunatCpe(models.Model):
	...
	company_id = fields.Many2one('res.company', string='Empresa', change_default=True, required=True, readonly=True, default=lambda self: self.env.company)
	...
	def action_draft(self):
		# Al regresar a borrador un comprobante sync NO aceptado por SUNAT,
		# descartar el XML/firma previos para forzar la regeneración limpia con
		# la estructura UBL corregida (mismo criterio que rc/ra en send_cpe).
		# GUARDA: nunca se toca un documento ya aceptado (estado_sunat '05' / state 'done').
		if self.type == "sync" and self.state != 'done' and self.estado_sunat != '05' and self.xml_document:
			self.xml_document = False
			self.datas = False
			self.datas_sign = False
	...
	# Cargar boletas al resumen
	def action_load_invoices(self):
		self.ensure_one()
		if self.type != 'rc':
			raise UserError('Esta acción solo está disponible para resúmenes de boletas.')
		if self.state != 'draft':
			raise UserError('Solo puede cargar boletas en resúmenes en estado borrador.')

		# Buscar boletas del día que no estén en ningún resumen
		invoices = self.env['account.move'].search([
			('invoice_date', '=', self.date),
			('state', '=', 'posted'),
			('company_id', '=', self.company_id.id),
			'|',
			('l10n_latam_document_type_id.code', '=', '03'),
			'&',
			('l10n_latam_document_type_id.code', 'in', ['07', '08']),
			('origin_doc_code', '=', '03'),
			('pe_summary_id', '=', False),
			('l10n_latam_document_type_id.is_synchronous', '=', False),
		])

		if not invoices:
			raise UserError('No se encontraron boletas asíncronas del %s sin resumen asignado.' % self.date)

		# Asignar las boletas a este resumen
		for invoice in invoices:
			invoice.pe_summary_id = self.id
		...
		return {
			...
			'params': {
				...
				'message': 'Se agregaron %s boleta(s) al resumen.' % len(invoices),
				'type': 'success',
				'sticky': False,
			}
		}
	...
	def get_cpe_async(self, type, invoice_id, is_voided=False):
		...
		for cpe_id in cpe_ids:
			if cpe_id:
				# Verificar el limite segun el tipo
				if type == 'rc':
					ids_to_check = cpe_id.summary_ids
				elif type == 'ra':
					ids_to_check = cpe_id.voided_ids
				else:
					ids_to_check = cpe_id.invoice_ids
	...
	def get_document_name(self):
		...
		if not self.invoice_ids and not self.voided_ids and not self.summary_ids:
			if self.type == 'rc':
				raise UserError('No se encontraron boletas en este resumen. Por favor agregue boletas con fecha %s antes de generar el resumen.' % self.date)
			elif self.type == 'ra':
				raise UserError('No se encontraron documentos para anular en esta comunicación de baja.')
			else:
				raise UserError('No se encontraron documentos en este CPE.')
		if self.type == "sync":
	...
	def send_cpe(self):
		...
		if self.type == "sync" and self.name == "/":
			self.name = self.invoice_ids[0].l10n_latam_document_number
		elif self.type == "ra" and self.name == "/":
			# Para comunicaciones de baja: formato RA-YYYYMMDD-NNNN
			# Fecha de GENERACION (= FechaGeneracion) para que el nombre coincida con el XML y SUNAT no rechace (error 2346)
			fecha_doc = datetime.strptime(local_date, "%Y-%m-%d").strftime("%Y%m%d")
			# Correlativo por fecha de generación (busca por prefijo del nombre)
			domain = [('type', '=', 'ra'), ('name', '=like', 'RA-%s-%%' % fecha_doc), ('company_id', '=', self.company_id.id)]
			existing = self.search(domain, order='name desc', limit=1)
			if existing and existing.name:
				# Extraer el número del último registro
				try:
					last_num = int(existing.name.split('-')[-1])
					next_num = last_num + 1
				except:
					next_num = 1
			else:
				next_num = 1
			self.name = "RA-%s-%04d" % (fecha_doc, next_num)
		elif self.type == "rc" and self.name == "/":
			# Para resúmenes: formato RC-YYYYMMDD-NNNN
			# Fecha de GENERACION (= FechaGeneracion) para que el nombre coincida con el XML y SUNAT no rechace (error 2346)
			fecha_doc = datetime.strptime(local_date, "%Y-%m-%d").strftime("%Y%m%d")
			# Correlativo por fecha de generación (busca por prefijo del nombre)
			domain = [('type', '=', 'rc'), ('name', '=like', 'RC-%s-%%' % fecha_doc), ('company_id', '=', self.company_id.id)]
			existing = self.search(domain, order='name desc', limit=1)
			if existing and existing.name:
				# Extraer el número del último registro
				try:
				...
				next_num = 1
			self.name = "RC-%s-%04d" % (fecha_doc, next_num)
		file_name = self.get_document_name()
		if self.type in ["rc", "ra"]:
			self.xml_document = False  # Forzar regeneracion en reintentos
			self._prepare_cpe()
			self._sign_cpe()
			self.datas_fname = file_name+".xml"
			self.datas_sign_fname = file_name+".xml"
	...
	def send_rc(self):
		cpe_ids = self.search([('state', 'in', ['draft', 'generate', 'send', 'verify']), ('type', 'in', ['rc'])])
		for cpe_id in cpe_ids:
			try:
				if not cpe_id.ticket:
					if cpe_id.state == 'send':
						cpe_id.xml_document = False  # Forzar regeneracion en reintentos
	...
	def send_ra(self):
		cpe_ids = self.search([('state', 'in', ['draft', 'generate', 'send', 'verify']), ('type', 'in', ['ra'])])
		for cpe_id in cpe_ids:
			try:
				if not cpe_id.ticket:
					check = True
				...
				_logging.error("Error en send_ra para CPE %s: %s", cpe_id.name, str(e))
	...
	def tp_estado_ticket(self):
		...
		for cpe_id in cpe_ids:
			try:
				...
				_logging.error("Error en tp_estado_ticket para CPE %s: %s", cpe_id.name, str(e))
			...
			if cpe_id.invoice_ids:
				if cpe_id.invoice_ids[0].pe_invoice_code not in ["03", "07"] and cpe_id.invoice_ids[0].origin_doc_code not in ["03", "07"]:
					try:
						...
						_logging.error("Error en action_document_status para CPE %s: %s", cpe_id.name, str(e))
				if cpe_id.state != 'done':
					if cpe_id.invoice_ids[0].pe_invoice_code not in ["03", "07"] and cpe_id.invoice_ids[0].origin_doc_code not in ["03", "07"]:
						try:
							...
							_logging.error("Error en send_async_cpe para CPE %s: %s", cpe_id.name, str(e))
			...
			if cpe_id.state != 'done':
				if cpe_id.invoice_ids[0].pe_invoice_code in ["07"] and cpe_id.invoice_ids[0].origin_doc_code not in ["03"]:
					try:
						...
						_logging.error("Error en send_async_cpe_nc para CPE %s: %s", cpe_id.name, str(e))
